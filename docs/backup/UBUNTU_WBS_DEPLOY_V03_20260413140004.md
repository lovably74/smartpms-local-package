...
| 구분 | 설명 |
|------|------|
| 브라우저 접속 | `https://wbs.smartpmis.net:5443` (Nginx TLS, 사설 인증서) |
| Node(Express + 정적 + tRPC) | `127.0.0.1:8080` 만 수신 (외부에서 직접 8080 접속 불가) |
| 리버스 프록시 | Nginx가 HTTPS 종료 후 `http://127.0.0.1:8080` 로 전달 |
...
- 설치 위치: `/etc/nginx/ssl/wbs.smartpmis.net.crt`, `/etc/nginx/ssl/wbs.smartpmis.net.key`
...
- **SAN**: 기본은 `DNS:wbs.smartpmis.net`, `DNS:localhost`, `IP:127.0.0.1` 및 선택적으로 `SMARTPMS_SSL_IP`(예: `192.168.0.121`)로 IP 접속 시 이름 불일치 완화
...
동작: `smartpms` 소스 tar 압축 → SCP → 원격에서 압축 해제 → `scripts/remote-bootstrap.sh` → `scripts/deploy/remote-install.sh` (OpenSSL 사설 인증서, Nginx, systemd, `pnpm install`, `pnpm build`).
...
| `/opt/smartpms/.env.deploy` | `HOST=127.0.0.1`, `PORT=8080`, `TRUST_PROXY=1`, 공개 URL `https://wbs.smartpmis.net:5443` (`OAUTH_*`, `VITE_*`) |
| `/etc/nginx/conf.d/smartpms-wbs.conf` | `listen 5443 ssl`, `http2 on`, `proxy_pass` → 127.0.0.1:8080 |
...
openssl x509 -in /etc/nginx/ssl/wbs.smartpmis.net.crt -noout -subject -dates
```
