...
배포 스크립트가 매번 로컬 PC의 `smartpms/.env`를 올리면, 개발 PC용 `DB_HOST`(예: 사내망 `10.x.x.x`)가 서버에 덮어씌워져 **`journalctl -u smartpms`에 `connect ETIMEDOUT`**이 나고 목록/로그인 등 DB 조회가 실패한다.

- 확인: `journalctl -u smartpms -n 50 --no-pager`
- 재발 방지: 아래 **`.env` 업로드 정책** 참고.
...
- **기본**: 로컬 `smartpms/.env`를 서버로 **올리지 않는다**. 서버에 이미 있는 `/opt/smartpms/.env`(DB·JWT)를 유지한다.
- **최초 1회** 또는 서버 `.env`를 로컱과 동기화할 때만 `-UploadEnv`를 지정한다.
